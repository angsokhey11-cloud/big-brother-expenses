/* BIG BROTHER — Expense History Mobile V1 */
(async function(){
'use strict';
...
function inject(){let doc,win;try{doc=frame.contentDocument||frame.contentWindow.document;win=frame.contentWindow}catch(_){return}if(!doc?.head||!doc?.body)return;
  if(!doc.getElementById('bb-expense-history-mobile-css')){const link=doc.createElement('link');link.id='bb-expense-history-mobile-css';link.rel='stylesheet';link.href='mobile-history.css?v=20260916-1';doc.head.appendChild(link)}
...
frame.src='index.html?embed=1&view=expense-history&mobileSkin=1&v=20260916-1';
